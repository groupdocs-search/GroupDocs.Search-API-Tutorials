...
**Download Direto:** Você também pode obter a biblioteca em [lançamentos do GroupDocs.Search para Java](https://releases.groupdocs.com/search/java/).
...
```
- **Parâmetros:** `basePath` aponta para a sua pasta de documentos; `basePort` é a porta TCP usada para comunicação entre nós.
...
```
- **Indexação Dinâmica:** Use o método `addDirectories` para **adicionar diretórios ao índice** dinamicamente sem reiniciar a rede.
...
```
- **Gerenciamento de Shards:** Lida eficientemente com grandes conjuntos de dados distribuindo documentos entre shards. Para **otimizar o tamanho do shard**, monitore as estatísticas dos shards e ajuste a configuração `shardSize` em versões futuras.
...
## Aplicações Práticas
1. **Gerenciamento Corporativo de Documentos:** Centralize a busca em milhões de arquivos.  
